...
const listIcon = {};
const listRow = { className: "list-item-container", querySelector: () => listIcon };
...
assert.match(source, /CONTENT_MESSAGE_SUFFIX = "_V74"/);
assert.match(contentSource, /__resumeAutofillContentProtocol/);
assert.match(contentSource, /CONTENT_PROTOCOL = 74/);
...
assert.match(contentSource, /for \(const target of multiSelector \? commitTargets : \[commitTarget\]\)/);
...
assert.match(contentSource, /const phoenixIndustry = phoenixSelectionCart && \/期望从事行业\/.test\(label\)/);
assert.match(contentSource, /plain-trusted-after-cart/);
